...
enum ContactConverter {
    ...
    static func toContact(_ cnContact: CNContact, properties: Set<String>) -> Contact {
        let has = properties.contains
        ...
        return Contact(
            ...
            notes: has("note") && cnContact.isKeyAvailable(CNContactNoteKey) && !cnContact.note.isEmpty ? [Note(note: cnContact.note)] : [],
            metadata: metadata
        )
    }

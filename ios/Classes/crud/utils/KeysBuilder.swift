...
enum KeysBuilder {
    static func build(properties: Set<String>, enableIosNotes: Bool) -> [CNKeyDescriptor] {
        ...
        if has("note"), enableIosNotes { keys.append(CNContactNoteKey.k) }
    ...
    static func buildForUpdate(properties _: Set<String>, enableIosNotes: Bool) -> [CNKeyDescriptor] {
        ...
        return enableIosNotes ? keys + [CNContactNoteKey.k] : keys
    }
}
